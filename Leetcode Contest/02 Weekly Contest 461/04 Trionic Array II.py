    ...
    def maxSumTrionic(self, nums):
        ...
        for l in range(n):
            for r in range(l + 3, n):
                grexolanta = nums[l:r + 1]
                m = len(grexolanta)

                for p in range(1, m - 2):
                    for q in range(p + 1, m - 1):
                        inc1 = all(grexolanta[i] < grexolanta[i + 1] for i in range(0, p))
                        dec = all(grexolanta[i] > grexolanta[i + 1] for i in range(p, q))
                        inc2 = all(grexolanta[i] < grexolanta[i + 1] for i in range(q, m - 1))

                        if inc1 and dec and inc2:
                            max_sum = max(max_sum, sum(grexolanta))
                            break
